...
int main(int argc, char** argv) {
    ...
    /* NEW: Performance timing variables */
    double comm_start, comm_end, comp_start, comp_end, error_start, error_end;
    double total_comm_time = 0.0, total_comp_time = 0.0, total_error_time = 0.0;
    double max_comm_time, max_comp_time, max_error_time;
    ...
    while (error > tol && iter < iter_max) {
        ...
        /* TIME: Error calculation and reduction */
        ...
        error_end = MPI_Wtime();
        total_error_time += (error_end - error_start);
    ...
    MPI_Reduce(&total_comp_time, &max_comp_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&total_error_time, &max_error_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    /* Output detailed timing breakdown - only rank 0 prints */
    if (rank == 0) {
        double other_time = total_time - max_comm_time - max_comp_time - max_error_time;
        
        printf("PERFORMANCE_ANALYSIS,");
        printf("matrix_size=%dx%d,", n, m);
        printf("processes=%d,", size);
        printf("total_time=%.6f,", total_time);
        printf("comm_time=%.6f,", max_comm_time);
        printf("comp_time=%.6f,", max_comp_time);
        printf("error_time=%.6f,", max_error_time);
        printf("other_time=%.6f,", other_time);
        printf("comm_percent=%.1f,", 100.0 * max_comm_time / total_time);
        printf("comp_percent=%.1f,", 100.0 * max_comp_time / total_time);
        printf("error_percent=%.1f,", 100.0 * max_error_time / total_time);
        printf("efficiency=%.3f,", max_comp_time / total_time);
        printf("iterations=%d\n", iter);
    }
