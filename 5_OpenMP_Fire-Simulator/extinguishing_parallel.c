...
int main(int argc, char *argv[])
{
	...
	// =================== TIMING VARIABLES ============================
	// =================================================================
	double time_total_iterations = 0.0;
	double time_loop_a = 0.0, time_loop_b = 0.0, time_loop_c = 0.0, time_loop_d = 0.0;
	double start_time, end_time; // Reused for timing each iteration
	double start_loop, end_loop; // Reused for timing each inner loop
	// =================================================================
	
	for (iter = 0; iter < max_iter && !flag_stability; iter++)
	{

		/* NEW: Start timer for the entire iteration body */
		start_time = cp_Wtime();

		/* 4.1. Activate focal points */
		int num_deactivated = 0;

		/* NEW: Start timer for loop 'a' */
		start_loop = cp_Wtime();
		for (i = 0; i < num_focal; i++)
		{
		...
		/* NEW: Stop timer for loop 'a' and accumulate the result */
		time_loop_a += cp_Wtime() - start_loop;

		/* 4.2. Propagate heat (10 steps per each team movement) */
		float global_residual = 0.0f;
		int step;

		/* NEW: Start timer for loop 'b' */
		start_loop = cp_Wtime();
		for (step = 0; step < 10; step++)
		{
			...
			for (i = 1; i < rows - 1; i++)
				for (j = 1; j < columns - 1; j++)
					accessMat(surface, i, j) = (accessMat(surfaceCopy, i - 1, j) +
			...
			#pragma omp parallel for collapse(2) private(i,j) reduction(max:global_residual)
			for (i = 1; i < rows - 1; i++)
				for (j = 1; j < columns - 1; j++)
				{
					float local_residual = fabs(accessMat(surface, i, j) - accessMat(surfaceCopy, i, j));
					if (local_residual > global_residual)
						global_residual = local_residual;
				}
		}
		/* NEW: Stop timer for loop 'b' and accumulate the result */
		time_loop_b += cp_Wtime() - start_loop;
		...
		/* NEW: Stop timer for loop 'c' and accumulate the result */
		time_loop_c += cp_Wtime() - start_loop;

		/* 4.4. Team actions */
		/* NEW: Start timer for loop 'd' */
		start_loop = cp_Wtime();
		for (t = 0; t < num_teams; t++)
		{
		...
		/* NEW: Stop timer for loop 'd' and accumulate the result */
		time_loop_d += cp_Wtime() - start_loop;

		/* NEW: Stop timer for the entire iteration and accumulate the result */
		time_total_iterations += cp_Wtime() - start_time;
	...
	printf("\n");
	printf("----------------------------------------------------\n");
	printf("           PERFORMANCE PROFILING RESULTS\n");
	printf("----------------------------------------------------\n");
	printf("Total time spent in main iterations: %.6f seconds\n\n", time_total_iterations);
	printf("Time in Loop a (Activate Focal): %.6f s  (%.2f %%)\n", time_loop_a, (time_loop_a / time_total_iterations) * 100.0);
	printf("Time in Loop b (Propagate Heat): %.6f s  (%.2f %%)\n", time_loop_b, (time_loop_b / time_total_iterations) * 100.0);
	printf("Time in Loop c (Move Teams):     %.6f s  (%.2f %%)\n", time_loop_c, (time_loop_c / time_total_iterations) * 100.0);
	printf("Time in Loop d (Team Actions):   %.6f s  (%.2f %%)\n", time_loop_d, (time_loop_d / time_total_iterations) * 100.0);
	printf("----------------------------------------------------\n");
	// =================================================================
