// OpenMP parallelized Laplace solver - Version 2 Simple
// Reduces overhead by fusing computation and update, keeping error separate
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char** argv) {
  ...
  while (error > tol && iter < iter_max) {
    ...
    // FUSED: Compute stencil AND update in one parallel region
    ...
    // Separate error calculation (most expensive operation)
    error = 0.0f;
    ...
    // Simple copy back - could be fused with next iteration's stencil
    #pragma omp parallel for collapse(2)
    for (i = 1; i < n - 1; i++) {
      for (j = 1; j < m - 1; j++) {
        A[i][j] = Anew[i][j];
